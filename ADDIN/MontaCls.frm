...
Begin VB.Form FrmMontaCls 
   ...
   Begin VB.TextBox TxtSuperClasse 
      ...
      Width           =   3555
   End
   Begin VB.ListBox LstDscExclusao 
      Height          =   5235
...
'            If DscExclusao = "" Then
               Print #1, "   Excluir = mvar" & NmDbObj & ".Executa(Me.QryDelete(" & ChaveParam & "), ComCOMMIT)"
'            Else
...
'            End If
         ...
         Else
            ...
            Print #1, "   Dim Sql As String"
            Print #1, "   On Error GoTo Fim"
            Print #1, "   Sql = QryDelete(" & ChaveParam & ")"
            Print #1, "   mvar" & NmDbObj & ".Execute Sql, dbExecDirect"
   ...
   Select Case Index
      Case 0
         ...
         If ValidaCampos Then
            For i = 0 To Me.LstTabela.ListCount - 1
               If Me.LstTabela.Selected(i) Then
                  Me.LstTabela.ListIndex = i
                  If Me.LstTabela.ItemData(i) > 0 Then
                     DscExclusao = Me.LstDscExclusao.List(Me.LstTabela.ItemData(i))
                  Else
                     DscExclusao = ""
                  End If
                  'On Error Resume Next
                  Call MontarClasse(Me.LstTabela.List(i), DscExclusao$)
                  j = j + 1
                  If j = Me.LstTabela.SelCount Then Exit For
               End If
            Next
...
Private Sub Lbl_Click(Index As Integer)
   ...
   End Select
End Sub
Private Sub LstDscExclusao_Click()
   Dim i As Integer
   If Not Me.LstTabela.Selected(Me.LstTabela.ListIndex) Then Exit Sub
   Me.LstTabela.ItemData(Me.LstTabela.ListIndex) = Me.LstDscExclusao.ListIndex
   If Me.LstDscExclusao.SelCount > 1 Then
      For i = 0 To Me.LstDscExclusao.ListCount - 1
         If Me.LstDscExclusao.Selected(i) And Me.LstDscExclusao.ListIndex <> i Then
            Me.LstDscExclusao.Selected(i) = False
         End If
         If Me.LstDscExclusao.SelCount = 1 Then Exit For
      Next
   ElseIf Me.LstDscExclusao.SelCount = 0 Then Me.LstDscExclusao.Selected(0) = False
   End If
End Sub
Private Sub LstDscExclusao_ItemCheck(Item As Integer)
   If Not Me.LstTabela.Selected(Me.LstTabela.ListIndex) Then
      Call ExibirAviso("Selecione a Tabela.", LoadMsg(1))
      Me.LstDscExclusao.Selected(Me.LstDscExclusao.ListIndex) = False
   End If
End Sub
...
'* Montar Combo de Campo de Descrição
   ...
   Me.ChkSelectAll.Value = vbUnchecked
   Me.LstDscExclusao.Clear
   If Me.LstTabela.Selected(Me.LstTabela.ListIndex) Then
      Me.LstDscExclusao.AddItem "  -- Em Branco -- "
   End If
   If UserDB = "" Then
      Tabela = Me.LstTabela
   Else
...
On Error Resume Next
   With DB.dBase.TableDefs(Tabela)
       For i = 0 To .Fields.Count - 1
         If .Fields(i).Attributes < dbSystemField Then
            Me.LstDscExclusao.AddItem .Fields(i).Name
         End If
      Next
   End With
   If Me.LstTabela.Selected(Me.LstTabela.ListIndex) Then
      Me.LstDscExclusao.Selected(Me.LstTabela.ItemData(Me.LstTabela.ListIndex)) = True
   End If
End Sub
...
Private Sub OptInPrj_Click(Index As Integer)
   ...
   Me.Refresh
   Bool = (Index = 0)
   Me.Lbl(2).Enabled = Bool
   Me.Lbl(3).Enabled = Bool
      ...
      For i = 0 To .TableDefs.Count - 1
         If (.TableDefs(i).Attributes And dbSystemObject) = 0 Then
            ...
            If UserDB = MyOwner Then
               Me.LstTabela.AddItem Mid(.TableDefs(i).Name, Pos + 1)
            End If
         End If
      Next
   End With
