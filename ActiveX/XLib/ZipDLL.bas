...
Option Explicit
Public NumArquivos      As Long    ' Número de Arquivos a compactar
...
'XXX        frmBarraProgress.barra.Min = 0
'XXX        frmBarraProgress.barra.Max = X
    End If
    For xx = 0 To X
        If fname.ch(xx) = 0 Then xx = 99999 Else s0 = s0 + Chr(fname.ch(xx))
'XXX        If NumArquivos > 0 Then frmBarraProgress.barra.Value = frmBarraProgress.barra.Value + 1
    Next xx
    If NumArquivos > 0 Then
'XXX        frmBarraProgress.informa.Caption = sVbZipInf & s0
'XXX        frmBarraProgress.barraGeral.Value = frmBarraProgress.barraGeral.Value + 1
     End If
...
'XXX        frmBarraProgress.barra.Value = frmBarraProgress.barra.Value + 1
    Next
    
'XXX    frmBarraProgress.informa.Caption = uZipInfo & s0
'XXX    frmBarraProgress.barraGeral.Value = frmBarraProgress.barraGeral.Value + 1
